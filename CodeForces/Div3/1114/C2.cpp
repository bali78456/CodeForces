    ...
    while(t--)
    {
        ...
        else
        {
            ...
            for(int i = 0; i < n;i+=2)
            {
                ...
                {
                    evenA1++;
                    evenAIdx.push_back(i);
                }
                if(b[i] == '1') 
                {
                    evenB1++;
                    evenBIdx.push_back(i);
                }
            }
            ...
            if(oddA1 == oddB1 && evenA1 == evenB1) {
                for(int i = 0; i < evenA1;i++)
                {
                    cnt += abs(evenAIdx[i] - evenBIdx[i])/2;
                }   
                for(int i = 0; i < oddA1;i++)
                {
                    cnt += abs(oddAIdx[i] - oddBIdx[i])/2;
                }
                cout << cnt << "\n";
            } else {
                cout << -1 << "\n";
...
// 1. 각각 홀 짝 확인은 똑같이 하고 서로 떨어져있는 인덱스만큼 (사이에 껴있는 원소갯수)
// 2. 처음부터 같으면 0출력, 기존로직으로 확인해보고 안되면 -1
// 3. 인덱스차이가 1이라면 1 더하고
// 4. 그 이상이면 인덱스차 - 1 만큼 더함
